...
def transform(**context):
    ...
    data['external_id'] = data['external_id'].apply(int)
...
def generate_gcs_remote_path(**context):
    ...
    return 'tripadvisor/MBL_SUMMARY/{}/{}/{}'.format(dates[0], dates[1], file_name)


def generate_bigquery_table_name(**context):
    short_date = context['task_instance'].xcom_pull(task_ids='sanity_check', key='short_date')
    return 'mbl_summary_{}'.format(short_date.replace('-', ''))
...
dag = DAG(
    dag_id='tripadvisor_mbl_summary_report',
...
s3_listdir_operation = S3ListDir(
    ...
    path='metasearchSFTP/tripadvisor/MBL_SUMMARY/Mbl_Summary_All_Ips-Hotelquickly-7-',
    dag=dag)
...
s3_upload_operator = S3Upload(
    ...
    remote_dir='metasearchSFTP/tripadvisor/MBL_SUMMARY',
    dag=dag)
...
bigquery_load_operator = BigQueryLoad(
    ...
    schema=[SchemaField('date', 'DATE'),
            ...
            SchemaField('external_id', 'INTEGER'),
            SchemaField('total_beats', 'INTEGER'),
            SchemaField('total_meets', 'INTEGER'),
            SchemaField('total_available', 'INTEGER'),
            SchemaField('total_requests', 'INTEGER'),
            SchemaField('percent_wins', 'FLOAT'),
            SchemaField('percent_available', 'FLOAT'),
            SchemaField('percent_beats', 'FLOAT'),
            SchemaField('percent_meets', 'FLOAT')],
