...
PARENT_DAG = 'tripadvisor_click_cost_ver2'
dag = DAG(
    dag_id=PARENT_DAG,
...
def transform(**context):
    ...
    temp_file_path_gzip = 'ClickCost-V04-HotelQuickly-' + short_date.replace('-', '_') + '.csv.gz'
    ...
    df = pd.read_csv(temp_file_path_zip, names=column_name, compression='zip', delimiter=',', quoting=csv.QUOTE_NONE,
                     header=16)
    df = df[df['report_date'] == short_date]
...
def generate_gcs_remote_path(**context):
    ...
    return 'tripadvisor/CLICKCOST_DAILY/{}/{}/{}'.format(dates[0], dates[1], file_name)


def generate_bigquery_table_name(**context):
    short_date = context['task_instance'].xcom_pull(task_ids='sanity_check', key='short_date')
    shorter_date = short_date.replace('-', '')
    return 'click_cost_{}'.format(shorter_date)
...
gcs_listdir_operation = GCSListDir(
    ...
    path='tripadvisor/CLICKCOST_DAILY',
    startswith='ClickCost-V04-HotelQuickly-',
    dag=dag)
